...
- Only processes events with the private `x-ctfbot-task` extended property
  marker, so foreign events are never imported as tasks.
...
2. **OAuth 2.0 credentials** — create an OAuth 2.0 Client ID (Desktop app
   type). Copy the client ID and client secret.

3. **Shared Calendar ID** — create or choose a Google Calendar that the bot
   will own. Copy its ID (find it in Calendar settings → Integrate calendar →
   Calendar ID).

4. **Refresh token** — visit the authorization URL below (generated by the bot
   or manually) with the calendar scope to obtain a one-time code, then exchange
...
- `src/lib/google/oauth.js` — OAuth 2.0 token refresh (fetch-based, no extra
  dependencies).
- `src/lib/google/calendar.js` — Google Calendar API v3 REST wrappers and
  mapper functions (`taskToCalendarEvent`, `calendarEventToTaskPatch`).
- `src/services/calendarSync.js` — poll loop (60s interval), push logic,
  incremental pull reconciliation.
- `migrations/019_add_calendar_sync.sql` — adds `calendar_event_id`,
  `calendar_synced_at`, `calendar_source` columns to `tasks`, plus
  `google_credentials` and `calendar_sync_state` tables.
